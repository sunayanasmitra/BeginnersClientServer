...
function _t$(doc, id) { if (id == '')
    return null; return doc.getElementById(id); }
function _s$(id) { if (id == '')
    return null; return document.getElementById(id); }
/*
class Model {
    mod:string;
    constructor(){
...
class Router {
    pthCopy; /* DO NOT DELETE.....*/
    prmCopy; /* DO NOT DELETE.....*/
    routes;
    functs;
    constructor(routes, functs) {
    ...
    launchFunc(func, param, urlsplit) {
        var ret = [], cnt = 0, cnt1 = 0, ignoreprocess = 0, strtparam = 0;
...
var the_util = {
    ...
    sett: null,
    util: {
        loadTemplateHandleFile: (url, data, func, tmplid) => {
    ...
    getFile: function (url, succfunc, rel, datatype) {
        let fullpath = "";
        if (!datatype)
            datatype = 'text';
    ...
    displayPage: function (pagecode, param) {
        const pg = this.sett.pages[pagecode];
        if (!param)
            param = {};
    ...
    jumpToPage: function (restcode, param) {
        if (!param)
            param = "";
        const pth = restcode + "/" + param;
        this.rout.navigate(pth, { trigger: true });
    },
    urlReplace: function (restcode, param) {
        if (!param)
            param = "";
        const pth = restcode + "/" + param;
        this.rout.navigate(pth, { replace: true });
    },
    requestData: function (url, paramstr, succfunc, method, rel) {
        let fullpath = "";
        if (!method)
            method = "POST";
        if (rel) { //relative path
