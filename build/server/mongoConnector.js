...
class MongoDBConnector {
    ...
    mongoClose(host) {
        if (this.mongoHost[host])
            this.mongoHost[host].close();
        this.mongoHost[host] = null;
    }
    getMongoCollection(db, coll) {
        return db.collection(coll);
    }
    async findMongoRec(db, monColl, callback, filter, options) {
        if (!filter)
            filter = {}; //if(!options) options ={};
    ...
    mongoColl(db, monColl, callback) {
        callback(db.collection(monColl));
    }
    async addMongoRec(db, monColl, docArr, callback, options) {
        let ret = { acknowledged: false };
        if (!options)
            options = {};
    ...
    async addIndex(db, monColl, indx, callback, options) {
        if (!options)
            options = {};
        let ret = { acknowledged: false };
        let coll = await db.collection(monColl);
    ...
    async updateMongoRec(db, monColl, key, upDoc, callback, options) {
        let coll = await db.collection(monColl);
        try {
    ...
    async deleteMongoRec(db, monColl, callback, filter, options) {
        if (!filter)
            filter = {};
        let coll = await db.collection(monColl);
        try {
