...
class MySQLConnector {
  ...
  getMySqlConn(connction:any,callback:any){   
    var err=[];
  ...
  selectAddlimit(selectstr,offset,orderby="",rowsperpage=-1,pagenum=1) {
  	let query = selectstr; query += (orderby !="") ? orderby : "";
  	if(rowsperpage > -1){
  ...
  setDynSql(sql:string,data:any,dupflag?:string,data2?:any){//dupflag: N=no on duplicate, S=string, V=value
    if(!dupflag || dupflag=='') dupflag='N';
    var outstmt='';var setp = sql.toLowerCase().indexOf("set");
    ...
    if(setp != -1) { // found
      ...
      if(typ=='insert'){
          var keyik = Object.keys(data);var lenik = keyik.length,cntik=1;
          outstmt +=stmt + " ( ";
          keyik.forEach(function(keyi1){
            outstmt +=keyi1;
            if(cntik < lenik) outstmt +=",";
            cntik++;
          });
          var keyiv = Object.keys(data);var leniv = keyiv.length,cntiv=1;
          outstmt += " ) values ( ";
      ...
      if(typ =='update' || dupflag.toLowerCase()=='v'){
          ...
          var keyu = Object.keys(data);var lenu = keyu.length,cntu=1; 
